...
\includegraphics[scale=0.5]{pictures/data_flow.PNG}
\subsection{Package diagram}
\includegraphics[scale=0.7]{pictures/package.png}
\subsection{Sequence diagram}
\includegraphics[scale=0.7]{pictures/sequence.PNG}

\newpage

\subsection{Diktat coding convention}
...
1.	All identifiers should use only ASCII letters or digits, and the names should match regular expressions `{2,64}`.

Explanation: Each valid identifier name should match the regular expression `{2,64}`.
...
- If the package name starts with a number or other character that cannot be used at the beginning of the Java/Kotlin package name, then underscores are allowed. For example: `com.example.\textunderscore 123name`.

- Underscores are sometimes permitted if the package name contains reserved Java/Kotlin keywords, such as `org.example.hyphenated\textunderscore name`, `int\textunderscore .example`.
...
2.	An underscore (`\textunderscore `) can be included in the JUnit test function name and should be used as a separator. Each logical part is denoted in `lowerCamelCase`, for example, a typical pattern of using underscore: `pop\textunderscore emptyStack`.
...
Magic constants, such as `NUM\textunderscore FIVE = 5` or `NUM\textunderscore 5 = 5` should not be treated as constants. This is because mistakes will easily be made if they are changed to `NUM\textunderscore 5 = 50` or 55.
...
\newpage 
\subsection{Available Inspections}
\scriptsize
