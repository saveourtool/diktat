...
enum class Warnings(
    ...
    fun warn(configs: List<RulesConfig>,
             ...
             offset: Int,
             node: ASTNode) {
        if (isRuleFromActiveChapter(configs) && configs.isRuleEnabled(this) && !node.hasSuppress(name, this, configs)) {
    ...
    private inline fun fix(
        ...
        node: ASTNode,
        autoFix: () -> Unit) {
        if (isRuleFromActiveChapter(configs) && configs.isRuleEnabled(this) && isFix && !node.hasSuppress(name, this, configs)) {
            autoFix()
        }
    }
