...
class DiktatSmokeTest : DiktatSmokeTestBase() {
    ...
    fun `smoke test with gradle script plugin`() {
        ...
        Assertions.assertEquals(
            LintError(2, 1, "${CommentsRule.NAME_ID}", "[COMMENTED_OUT_CODE] you should not comment out code, " +
    ...
    fun `disable chapters`() {
        ...
        unfixedLintErrors.assertEquals(
            ...
            LintError(3, 1, "${KdocComments.NAME_ID}", "${MISSING_KDOC_TOP_LEVEL.warnText()} example", false),
            LintError(3, 16, "${KdocComments.NAME_ID}", "${MISSING_KDOC_CLASS_ELEMENTS.warnText()} isValid", false),
            LintError(6, 5, "${KdocComments.NAME_ID}", "${MISSING_KDOC_CLASS_ELEMENTS.warnText()} foo", false),
            LintError(6, 5, "${KdocMethods.NAME_ID}", "${MISSING_KDOC_ON_FUNCTION.warnText()} foo", false),
            LintError(8, 5, "${KdocComments.NAME_ID}", "${MISSING_KDOC_CLASS_ELEMENTS.warnText()} foo", false),
            LintError(10, 4, "${KdocFormatting.NAME_ID}", "${KDOC_NO_EMPTY_TAGS.warnText()} @return", false),
            LintError(13, 9, "${KdocFormatting.NAME_ID}", "${KDOC_NO_EMPTY_TAGS.warnText()} @return", false),
            LintError(18, 40, "${KdocFormatting.NAME_ID}", "${KDOC_NO_EMPTY_TAGS.warnText()} @return", false)
        )
    }
